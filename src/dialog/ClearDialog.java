...
/* A Jdialog that appears after clearing the game.
...
public class ClearDialog extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public ClearDialog(JFrame parent, int playTime, int score, int difficulty) {
		super(parent, true); // The parent argument contains the JFrame of the running GameGUI.
		getContentPane().setBackground(Color.DARK_GRAY);
		this.setIconImage(Toolkit.getDefaultToolkit().getImage(ClearDialog.class.getResource("/ImageFile/Star.png")));
		this.setTitle("Stage Clear");
		...
		{ /* North of pnInformation */
			JLabel lblGameOver = new JLabel("Stage Clear");
		...
		{ /* Center of pnInformation */
			...
			pnRecord.setBackground(Color.WHITE);
			pnInformation.add(pnRecord, BorderLayout.CENTER);
			pnRecord.setLayout(null);
			{
				// Used html language to write spaces in JLabel.
				String info = String.format(
						"<html><center>Play Time %d:%02d<br>Score %d</center><html>", playTime / 60, playTime % 60, score);
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (!textFieldName.getText().isEmpty()) {
						try { /* Exception Handling */
							...
							JOptionPane.showMessageDialog(ClearDialog.this, "Special characters cannot be used.",
	...
	/* A Method that displays ClearDialog and returns the button selected by the user */
	public int showClearDialog() {
		this.setVisible(true);
		return this.clickOption; // YES or NO
	} // end method showClearDialog
} // end class ClearDialog
